...
for (iii in c(1:nrow(species_evo_types))){
  ...
  df_list[[paste(sp_item$species_evo_type, sp_item$directional_speed, target)]]<-df
}
saveRDS(df_list, "../Data/Example/%s.dis.rda")
saveRDS(tree_list, "../Data/Example/%s.tree.rda")
n<-names(df_list)[1]
i=349
for (i in c(max(df$year):min(df$year))){
  ...
  pp1<-ggarrange(plist[[1]], ggarrange(plotlist=plist[c(2:13)], nrow=4, ncol=3),
                 widths = c(1,1), align = "v")
  ggsave(pp1, filename=sprintf("../Figures/Example/all_scenarios/%d.png", i),
         width=10.5, height=8)
}
