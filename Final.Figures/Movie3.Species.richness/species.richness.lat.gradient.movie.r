...
for (y in c(1199:0)){
  print(y)
  targetxxx<-sprintf("../Figures/Movie3.Species.Richness/years_lat_unique/%d.png", y)
  ...
  if (y==1199){
    ...
    diversity_final[mean_N_SPECIES>1]$mean_N_SPECIES<-1
    ...
    diversity_final2<-readRDS(sprintf("../Data/diversity/diversity_bootstrap_years_unique/%d.rda", y))
    colnames(diversity_final2)[5:8]<-c("mean_N_SPECIES2",  "medium_N_SPECIES2", "ci2", "sd_N_SPECIES2" )
    diversity_final<-merge(diversity_final, diversity_final2, by=c("global_id", "species_evo_type", "directional_speed", "year", "group"),
                           all.x=T)
    diversity_final[is.na(mean_N_SPECIES2)]$mean_N_SPECIES2<-0
    diversity_final$mean_N_SPECIES <- diversity_final$mean_N_SPECIES - diversity_final$mean_N_SPECIES2
    diversity_final$medium_N_SPECIES<-diversity_final$medium_N_SPECIES - diversity_final$medium_N_SPECIES2
    diversity_final[mean_N_SPECIES<0]$mean_N_SPECIES<-0
    is_uniform<-F
  }
  ...
  p1<-create_fig_map(diversity_final[group=="conservatism"], evo_types_label_line[1], polygon=NULL, 
  ...
  pp<-ggarrange(plotlist=list(p1, p2, p3, p5), nrow=4, 
                heights = c(1.5, 1,1,1))
  pp<-annotate_figure(pp, fig.lab.pos = "top.right", 
...
#cd /media/huijieqiao/Butterfly/Niche_Conservatism/Figures/Movie3.Species.Richness/years_lat_unique
#ls | xargs -I {} mv {} y-{}

#ffmpeg -r 20 -start_number -1200 -i y%d.png -y ../species.richness.lat.mp4
#ffmpeg -r 20 -start_number -1200 -i y%d.png -y ../species.richness.lat.mp4 -c:v libx264 -profile:v baseline -c:a aac -ar 44100 -ac 2 -b:a 128k-vf format=yuv420p
