...
if (F){
  ...
  seeds_all<-readRDS("../Data/seeds.rda")
  outliers<-readRDS("../Data/outliers/outliers_3SD.rda")
  seeds_all<-seeds_all[!(global_id %in% outliers$global_id)]
  template<-"%d_%s_%s_%d_%s_%d"
  j=1
  i=1
  base<-"/media/huijieqiao/QNAS/Niche_Conservatism/Results"
  ...
  i=1
  for (i in c(1:nrow(seeds_all))){
    seed<-seeds_all[i]
    
    for (j in c(1:nrow(combinations))){
      
      com<-combinations[j]
      print(paste(i, nrow(seeds_all), j, nrow(combinations),
                  seed$global_id, com$da, com$nb, com$species_evo_type, 
                  com$directional_speed))
      sp<-sprintf(template, seed$global_id, com$da, com$nb, com$species_evo_type, 
                  com$directional_speed, 0)
  ...
  target<-"../Data/diversity_items_full_year"
  for (y in c(0:1198)){
    setwd("/media/huijieqiao/Butterfly/Niche_Conservatism/RScript")
    print(paste("Saving ", y))
    dddd<-d_year[[as.character(y)]]
    ...
    diversity_se<-dddd[, .(N_SPECIES=sum(N_SPECIES), N_INDIVIDUAL=sum(N_INDIVIDUAL)),
                       by=list(year, global_id, 
                               species_evo_type, directional_speed)]
    saveRDS(diversity_se, sprintf("%s/y%d.rda", target, y))
    diversity_se_nb<-dddd[, .(N_SPECIES=sum(N_SPECIES), N_INDIVIDUAL=sum(N_INDIVIDUAL)),
                          by=list(year, global_id, 
                                  species_evo_type, directional_speed, nb)]
    saveRDS(diversity_se_nb, sprintf("%s/y%d_nb.rda", target, y))
    diversity_se_nb_da<-dddd[, .(N_SPECIES=sum(N_SPECIES), N_INDIVIDUAL=sum(N_INDIVIDUAL)),
                             by=list(year, global_id, 
                                     species_evo_type, directional_speed, nb, da)]
    
    saveRDS(diversity_se_nb_da, sprintf("%s/y%d_nb_da.rda", target, y))
  }
...
if (F){
  ...
  for (y in c(0:1198)){
    ...
    dddd_nb<-list()
    dddd_nb_da<-list()
    targetxx<-sprintf("../Data/diversity/diversity_bootstrap_years_unique/%d.rda", y)
    if (file.exists(targetxx)){
      next()
    }
    saveRDS(NULL, targetxx)
    for (i in c(1:100)){
      print(paste(y, i))
      target<-sprintf("../Data/diversity_boostrap_items_full_year_unique/r_%d/y%d_r%d.rda", i, y, i)
      
      item<-readRDS(target)
      dddd[[length(dddd)+1]]<-item
      
      target_nb<-sprintf("../Data/diversity_boostrap_items_full_year_unique/r_%d/y%d_r%d_nb.rda", i, y, i)
      item_nb<-readRDS(target_nb)
      dddd_nb[[length(dddd_nb)+1]]<-item_nb
      
      target_nb_da<-sprintf("../Data/diversity_boostrap_items_full_year_unique/r_%d/y%d_r%d_nb_da.rda", i, y, i)
    ...
    saveRDS(dddd_se, sprintf("../Data/diversity/diversity_bootstrap_years_unique/%d_nb.rda", y))
    ...
    saveRDS(dddd_se, sprintf("../Data/diversity/diversity_bootstrap_years_unique/%d_nb_da.rda", y))
