...
for (i in c(1:nrow(all_df))){
  ...
  if (species_evo_level==0){
    if ((nb=="BROAD")&(da=="GOOD")){
      ttt<-sprintf("/media/huijieqiao/QNAS/Niche_Conservatism/Results/%s/%s.N.speciation.extinction.rda", sp, sp) 
    }else{
      ttt<-sprintf("/media/huijieqiao/QNAS/Niche_Conservatism/Results/%s/%s.N.speciation.extinction.rda", sp, sp)  
    }
  }else{
    if (item$species_evo_type==1){
      ...
      ttt<-sprintf("/media/huijieqiao/QNAS/Niche_Conservatism/Results_1/%s/%s.N.speciation.extinction.rda", sp, sp) 
    }
  }
  if (!file.exists(ttt)){
    #print("skip")
