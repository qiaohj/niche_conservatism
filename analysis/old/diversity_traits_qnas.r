...
for (i in c(1:nrow(all_df))){
  ...
  if (item$species_evo_level==1){
    ...
    if ((item$nb=="BROAD")&(item$da=="GOOD")){
      ...
      base_check<-"../Results"
      base<-"/media/huijieqiao/QNAS/Niche_Conservatism/Results_2"
      #base<-"/media/huijieqiao/NAS/Niche_EVO"
    }else{
      base<-"/media/huijieqiao/QNAS/Niche_Conservatism/Results"
    }
  }
  ttt<-sprintf("%s/%s/%s.diversity.rda", base_check, sp, sp)
  ...
  if (!file.exists(log_check)){
    print("not exist, skip")
    next()
  }
  
  
  if (file.size(log)!=file.size(log_check)){
    
    print("different size, skip")
    next()
  }
  if ((file.size(log)/1024/1024)>3500){
    print("too big, skip")
    #next()
  }
