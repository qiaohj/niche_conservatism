...
for (i in c(1:rep)){
  ...
  for (year in c(1200:0)){
    ...
    N_SPECIES<-sub_detail_df%>%dplyr::filter((TO<=year)&(FROM>year))%>%
      dplyr::group_by(DA, NB, EVO_TYPE, EVO_RATIO, WARP_LABEL, START_POINT)%>%
    ...
    item<-left_join(N_SPECIES, N_SPECIATION, by=c("DA", "NB", "EVO_TYPE", "EVO_RATIO", "WARP_LABEL", "START_POINT"))
    item<-left_join(item, N_EXTINCTION, by=c("DA", "NB", "EVO_TYPE", "EVO_RATIO", "WARP_LABEL", "START_POINT"))
    item[is.na(item)]<-0
    item$YEAR<-year
    item$REP<-i
